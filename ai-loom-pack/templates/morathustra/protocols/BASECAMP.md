...
Every Morathustra Basecamp command must include literal `--profile morathustra`
...
Use `loom-morathustra-basecamp` for named task calls, with the command directly
...
credentials. Morathustra must never use the operator's, Codex's, Apollo's or any
...
account setup, seat changes and first external writes require separate
integrator authorization with the operator present. An invitation or a matching
display name is not proof of identity or account authority.

During that separately authorized bootstrap only, the integrator runs the
identity probe above after successful OAuth and verifies the dedicated
Morathustra actor and the exact example-operator account with the operator. Capture and freeze
...
`Written by Morathustra` without copying another agent's signature.
