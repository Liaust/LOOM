...
The `loom-mina-gh` entry point and MINA operator binding are implemented.
Deployment and authenticated verification are required before task calls;
inspect the current binding and probe rather than assuming source means live.
This source grants no runtime rename, account change or GitHub publication.
An approved custody migration preserves the verified actor and isolated grant;
it does not request OAuth again or expand grants merely to change the name.

## Personal Identity And Isolated Authentication

Use the operator's personal GitHub identity through the approved isolated OAuth
grant on Main, established by a separately authorized fresh bootstrap. Never copy or reuse existing Mac/shared tokens, shared CLI login,
...
For named task calls use `loom-mina-gh`, with the command first and
options after it. Do not pass `--profile mina`: that is a Basecamp-only selector,
not a GitHub CLI flag. For example, `loom-mina-gh api user` performs the identity
probe. The wrapper binds the exact native CLI, isolated child HOME/config/cache
and GH_CONFIG_DIR, github.com and Nix's trusted CA bundle. Do not export those
settings in the parent shell. It discards inherited token/host/transport settings
before fixed dispatch. Explicit conflicting command options and unsafe or missing
auth metadata still refuse; no bootstrap or credential repair occurs.
...
current-user probe is `loom-mina-gh api user`; compare its structured
...
The private `OWNER/mina-workspace` repository, when separately created by the
operator, holds a positive source export. This template does not create it.
It is not the live workspace and does not automatically synchronize it.
Maintain it within the accepted task using reviewed portable source changes.
Verify exact owner, repository name, private visibility and the verified
personal actor's permission before every push. Creating another repository or
changing visibility is a separate action under `OPERATING-POLICY.md`.
...
MINA's personality, private memory or broader operational authority.
