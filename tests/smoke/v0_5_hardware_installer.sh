...
create_remote_tmp() {
  if [[ -n "$REMOTE_BASE" ]]; then
    remote_target "mkdir -p '$REMOTE_BASE' && mktemp -d '$REMOTE_BASE/v0-5-hardware.XXXXXX'"
  else
    remote_target 'base="${HOME:-/tmp}/.cache/loom-smoke"; mkdir -p "$base" && mktemp -d "$base/v0-5-hardware.XXXXXX"'
  fi
}
...
LOCAL_TMP="$(mktemp -d "${TMPDIR:-/tmp}/loom-v0-5-hardware.XXXXXX")"
cleanup() {
  rm -rf "$LOCAL_TMP"
  if [[ "$KEEP" != "1" ]]; then
...
FIRST_JSON="$LOCAL_TMP/hardware-first.json"
SECOND_JSON="$LOCAL_TMP/hardware-second.json"

run_loom --json bootstrap ssh "$TARGET_HOST" "${BOOTSTRAP_FLAGS[@]}" --yes >"$FIRST_JSON"
check_json_file "hardware SSH bootstrap enrolled node" "$FIRST_JSON" \
  '.status == "applied" and .enrollment.status == "verified_on_main" and .remote_apply.manifest.node_kind == "hardware" and (.remote_apply.manifest.box_path // "") == ""'

check_target "hardware installer created manifest and node-agent state without Box" "
  test -f '$MANIFEST'
  test -f '$NODE_AGENT_CONFIG'
  test -f '$NODE_AGENT_STATE'
  test ! -d '$HOME_DIR/LOOM Box'
"

check_target_json "hardware node-agent status shows credential configured" \
  "$REMOTE_AGENT --json status" \
  ".data.config.node_key == \"$NODE_KEY\" and .data.config.node_kind == \"hardware\" and .data.credential_configured == true"

check_target_json "hardware heartbeat succeeds from target" \
  "$REMOTE_AGENT --json heartbeat --once" \
  '.ok == true and .data.presence_state == "online"'

check_main_json "main can inspect hardware node profile" \
  "loom --json node inspect '$NODE_KEY'" \
  '.ok == true and .data.node_kind == "hardware" and .data.runtime_class == "hardware_agent"'
check_main_json "main can inspect hardware node health" \
  "loom --json node health '$NODE_KEY'" \
  '.ok == true'

run_loom --json bootstrap ssh "$TARGET_HOST" "${BOOTSTRAP_FLAGS[@]}" --yes >"$SECOND_JSON"
check_json_file "hardware SSH bootstrap rerun resumes existing enrollment" "$SECOND_JSON" \
...
check_target_json "hardware setup doctor reports no blocking findings" \
...
pass "hardware bootstrap output does not leak enrollment token patterns"
